/*
  ...
  02111-1307, USA.
*/
// $Id: EOKeyValueArchiver.m,v 1.3 2008/01/10 09:34:54 davelopper Exp $
...
@interface EOKeyValueUnarchiver(Private)
...
- (id)_decodeCurrentPlist {
  ...
  if ([self->plist isKindOfClass:[NSArray class]]) {
	...
	objs = NSZoneMalloc(NSDefaultMallocZone(), count * sizeof(id));
	for (i = 0; i < count; i++)
	  objs[i] = [self decodeObjectAtIndex:i];
	
	obj = [NSArray arrayWithObjects:objs count:count];
	NSZoneFree(NSDefaultMallocZone(), objs);
      }
      return obj;
  }
