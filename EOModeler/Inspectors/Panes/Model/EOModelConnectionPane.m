...
@implementation EOModelConnectionPane
...
{
	[info release];
	info = [[(EOModel *)[self selectedObject] connectionDictionary] mutableCopy];
...
- (void)selectRow:(id)sender
{
	...
	row = [keyValueTable selectedRow];
	if (row >= 0) {
		[valueText setString:[[info objectForKey:[keys objectAtIndex:row]] description]]; // Object might be a property list (NSDictionary, NSArray, etc.)
	} else {
		[valueText setString:@""];
	}
}
...
- (void)add:(id)sender
{
	...
	[[self selectedObject] setConnectionDictionary:info];
}
